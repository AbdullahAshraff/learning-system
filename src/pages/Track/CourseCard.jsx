...
import track from './../../constants/tracks.js'
...
const CourseCard = ({track}) => {
    ...
    return (
        <div className="border rounded-lg p-6 shadow-lg transition-transform transform hover:scale-100 hover:shadow-amber-700 relative bg-white overflow-hidden mx-5 max-w-xs group">
            ...
            />
            <h2 className="text-xl font-semibold text-gray-800 hover:text-[#DEB041] transition-colors duration-300">{track.title}</h2>
            <p className="text-gray-600 text-sm mb-2">{track.description}</p>
            <div className="mt-2 flex justify-between items-center text-sm">
            </div>
            <div className="mt-3 flex justify-between items-center text-lg font-bold text-[#DEB041]" onClick={() => handleTrackClick(track.id)}>
                <span 
